...
func (m *RequiredExample) LahsramOt(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
...
func (m *NestedNinOptNative) LahsramOt(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	...
	if len(m.NestedNinOpts) > 0 {
		for iNdEx := len(m.NestedNinOpts) - 1; iNdEx >= 0; iNdEx-- {
			{
				size, err := m.NestedNinOpts[iNdEx].LahsramOt(dAtA[:i])
				if err != nil {
					return 0, err
				}
				i -= size
