...
func (m *CustomMap) DeterministicMarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if len(m.Nullable128S) > 0 {
		keysForNullable128S := make([]string, 0, len(m.Nullable128S))
		for k := range m.Nullable128S {
			keysForNullable128S = append(keysForNullable128S, string(k))
		}
		github_com_gogo_protobuf_sortkeys.Strings(keysForNullable128S)
		for _, k := range keysForNullable128S {
			dAtA[i] = 0xa
			i++
			v := m.Nullable128S[string(k)]
			cSize := 0
			if v != nil {
				cSize = v.Size()
				cSize += 1 + sovMapsproto2(uint64(cSize))
	...
	if len(m.Uint128S) > 0 {
		keysForUint128S := make([]string, 0, len(m.Uint128S))
		for k := range m.Uint128S {
			keysForUint128S = append(keysForUint128S, string(k))
		}
		github_com_gogo_protobuf_sortkeys.Strings(keysForUint128S)
		for _, k := range keysForUint128S {
			dAtA[i] = 0x12
			i++
			v := m.Uint128S[string(k)]
			cSize := 0
			cSize = v.Size()
			cSize += 1 + sovMapsproto2(uint64(cSize))
			mapSize := 1 + len(k) + sovMapsproto2(uint64(len(k))) + cSize
	...
	if len(m.NullableIds) > 0 {
		keysForNullableIds := make([]string, 0, len(m.NullableIds))
		for k := range m.NullableIds {
			keysForNullableIds = append(keysForNullableIds, string(k))
		}
		github_com_gogo_protobuf_sortkeys.Strings(keysForNullableIds)
		for _, k := range keysForNullableIds {
			dAtA[i] = 0x1a
			i++
			v := m.NullableIds[string(k)]
			cSize := 0
			if v != nil {
				cSize = v.Size()
				cSize += 1 + sovMapsproto2(uint64(cSize))
	...
	if len(m.Ids) > 0 {
		keysForIds := make([]string, 0, len(m.Ids))
		for k := range m.Ids {
			keysForIds = append(keysForIds, string(k))
		}
		github_com_gogo_protobuf_sortkeys.Strings(keysForIds)
		for _, k := range keysForIds {
			dAtA[i] = 0x22
